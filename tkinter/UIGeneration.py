...
def generate_edit_object_frame(self):
    ...
    self.btn_discard = ttk.Button(self.edit_object_frame, text="Discard changes")
    ...
    for i in range(30):
        ttk.Label(self.sclbox_object_att, text=f"Attribut {i}", padding=10).pack(fill="x", padx = 10, pady = 5)
...
def generate_header_frame_ui(self):
    ...
    self.header_frame.grid_rowconfigure((1, 2), weight=1)
    ...
    self.btn_parallax = ttk.Button(self.header_frame, text="Add\nparallax")
    self.btn_parallax.grid(row=1, column=0, sticky="news", padx=(10,5), pady=5)

    self.btn_music = ttk.Button(self.header_frame, text="Add\nmusic")
    self.btn_music.grid(row=1, column=1, sticky="news", padx=5, pady=5)

    self.btn_load = ttk.Button(self.header_frame, text="Load a\nlevel")
